	...
	<div class="container d-flex align-items-center justify-content-center">
                  <div class="row">
                        ...
                        <div class="col-md-12 col-md-offset-0 col-sm-10 col-sm-offset-1">
                              <h2 style="text-align: center !important;">Ajouter Nouveau Grade</h2>
                              <form action="{{route('grades.store') }}" method="POST" validate>
                                    {{ csrf_field() }}
                                    <div class="form-group {{ $errors->has('nomGrade') ? 'has-error' : '' }}">
                                          <label for="nomGrade" class="control-label">Nom Du Grade</label>
                                          <input type="text" name="nomGrade" value="{{old('nomGrade')}}" id="nomGrade" class="form-control" required="required">
                                          {!! $errors->first('nomGrade', '<span class="help-block">:message</span>') !!}
                                    </div>
                                    <div class="form-group">
                                          <label for="codeGrade" class="control-label">Code Du Grade</label>
                                          <input type="text" name="codeGrade" value="{{old('codeGrade')}}" id="codeGrade" class="form-control" required="required">
                                    </div>
                                    <div class="form-group">
                                          <label for="salaireBaseGrade" class="control-label">Salaire De Base Du Grade</label>
                                          <input type="salaireBaseGrade" name="salaireBaseGrade" value="{{old('salaireBaseGrade')}}" id="salaireBaseGrade" class="form-control" required="required">
